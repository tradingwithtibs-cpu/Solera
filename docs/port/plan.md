...
- **Discover tabs rework** (Sept 23, on the user's spec): News = the last 24 hours newest first over market + Trending's top-held tickers + holdings (`GENERAL_LIMIT` 60 in `news-server.ts`); Hot = headlines only, votes with decay, ties to stories about held/trending tickers, capped at 40; Everyone = the `everyone` lobby room rendered by `RoomPanel` inside the card (`LOBBY_ROOM` in `lib/chat.ts`; **`supabase/lobby.sql` must run once** to relax the `messages.room` check); Following = followed people's Solera fills + their on-chain xStock swaps (`lib/activity.ts` parser, `GET /api/activity?wallets=` over the public RPC, 10-minute cache per wallet, 20 signatures each, `tests/activity.test.mjs`) + their room posts (`GET /api/chat?authors=`); Holdings (was Mine) = news + room posts for held tickers (`GET /api/chat?rooms=`). New: `src/hooks/use-chat-feed.ts`, `MessageRow.tsx`, `FeedMessage` row kind, `useFollowedInvestors().followed`, the `.feed-card` head wraps its tabs instead of overflowing.
