...
import { PYTH_FEEDS, hermesPriceToNumber, isUnderlyingStale, type HermesPrice } from "./pyth-feeds";
import { derivePriceUpdateAddress, parsePriceUpdateV2 } from "./pyth-onchain";
import { SOL, XSTOCK_TOKENS } from "./tokens";
...
/**
 * Live xStock and underlying-equity prices for every featured ticker, with
 * Pyth as the reference price for the underlying share. Server-only, so
 * keys never reach the browser, and shared by the /api/live-prices route
 * and the agent's price tool (which calls it in-process). One upstream
 * fetch is reused for CACHE_TTL_MS by everyone.
 */
export async function loadLivePrices(): Promise<LivePricesResponse | null> {
...
/** Non-featured tickers from the catalog, priced through Jupiter on demand (no cache). */
export async function priceExtraTickers(extra: string[]): Promise<Partial<Record<TickerSymbol, number>>> {
